...
class UsersController extends AdminController {
	...
	public function api_index()
	{
		$users = \DB::table('users')
			->select('username','email','phone','activated','created_at','id');
		return \Datatables::of($users)
			->edit_column('activated','@if(!$activated==0) <button class="btn btn-success"><i class="icon-ok"></i></button> @else <button class="btn btn-danger"><i class="icon-remove"></i></button> @endif')
			->edit_column('id','<a href="{{ route(\'update/user\', $id) }}" class="btn btn-primary btn-mini">编辑</a>    <a href="{{route(\'delete/user\', $id)}}" class="btn btn-danger btn-mini del">删除</a>')
			->make();
	}
	...
	public function create()
	{
		...
		$units = Sentry::getUnitProvider()->findAll();
		// Selected groups
		...
		// Show the page
		return View::make('admin.users.create', compact('groups','units','selectedGroups', 'permissions', 'selectedPermissions'));
	}
	...
	public function postCreate()
	{
		...
		try{
			...
			$inputs = Input::except('csrf_token','password_confirm','groups','unit');
			...
			if ($user = Sentry::getUserProvider()->create($inputs)) {
				...
				$unit = Sentry::getUnitProvider()->findById($unitId);
				$user->addUnit($unit);
	...
	public function getEdit($id=null)
	{
		...
		try
		{
			...
			$userUnits = $user->units()->lists('name', 'unit_id');
		...
		// Show the page
		return View::make('admin.users.edit', compact('user', 'groups', 'userGroups', 'permissions', 'userPermissions','units','userUnits'));
	}
	 ...
	 */
		...
		//
			...
			$this->validationRules['phone'] = "required|unique:users,phone,{$user->phone},phone";
		...
		// If validation fails, we'll exit the operation now.
			...
			// Do we want to update the user password?
				...
				// 判断账号传递来的是否为冻结 0:解除 1:冻结

				if (Input::get('suspend')=='0') {
					$throttle->unsuspend();
				}
				elseif (Input::get('suspend')=='1') {
					$throttle->suspend();
				}
	...
	public function getDelete($id = null)
	{
		...
		// Delete the user,Delete from users_units / users_groups
			//获取当前部门编号,并从部门中删除
			$userUnit = $user->units()->lists('unit_id');
			$currentUnitId = $userUnit[0];
			$unitToRemove = Sentry::getUnitProvider()->findById($currentUnitId);
