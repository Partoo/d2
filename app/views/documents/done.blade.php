...
<?php
    if ($data->state==3) {
      $docNow = round(0.5+0.25*(($data->comments->count()-1)/$recieversCount),2)*100;
    }elseif($data->state==4){$docNow=100;}
    else{
      $docNow = abs(round($data->state/4,2))*100;
    }
          ...
          <div class="post-comment well">
            ...
            {{Form::open()}}
             ...
             <textarea class="span12" name="comment" rows="8" placeholder="您可以在此填写、补充您的签收意见，但提交后将不能修改。"></textarea>
                      <p>{{Form::submit('提交办结',array('class'=>'btn btn-large btn-info'))}}  <button class="btn btn-large btn-danger disabled"><i class="icon-info-sign"></i> 应签收{{$recieversCount}}人，实签收{{$data->comments->count()-1}}人</button></p>
           {{Form::close()}}
         </div>
         @endif
