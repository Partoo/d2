...
use Cartalyst\Sentry\Units\Eloquent\Provider as UnitProvider;
use Cartalyst\Sentry\Units\ProviderInterface as UnitProviderInterface;
...
class Sentry {
	...
	protected $groupProvider;
                ...
                /**
     * The unit provider, used for retrieving
     * objects which implement the Sentry unit
     * interface.
     *
     * @var \Cartalyst\Sentry\Units\ProviderInterface
     */
	...
	public function __construct(
		...
		GroupProviderInterface $groupProvider = null,
                        UnitProviderInterface $unitProvider = null,
       ...
       )
	{
		...
		$this->groupProvider    = $groupProvider ?: new GroupProvider;
                        $this->unitProvider    = $unitProvider ?: new UnitProvider;
	...
	public function authenticate(array $credentials, $remember = false)
	{
		...
		{
			throw new LoginRequiredException("请输入您的手机或邮箱");
		}

		if (empty($credentials['password']))
		{
			throw new PasswordRequiredException('密码没有填写');
		}
	...
	public function check()
	{
		if (is_null($this->user))
		{
			...
			if ( ! is_array($userArray) or count($userArray) !== 3) //by Partoo :changed 2 to 3
			{
				return false;
			}

			// list($id, $persistCode) = $userArray; by partoo
           list($id, $persistCode, $loginName) = $userArray;
           $this->getUserProvider()->getEmptyUser()->setLoginAttributeName($loginName);
...
return true;
	...
	public function login(UserInterface $user, $remember = false)
	{
		...
		{
			$login = $user->getLogin();
			throw new UserNotActivatedException("该用户尚未激活");
		}

		$this->user = $user;

		// Create an array of data to persist to the session and / or cookie
		// $toPersist = array($user->getId(), $user->getPersistCode()); by Partoo
        $toPersist = array($user->getId(), $user->getPersistCode(), $user->getLoginName());
	...
	public function getGroupProvider()
	{
		return $this->groupProvider;
            ...
            /**
             * Sets the unit provider for Sentry.
             *
             * @param  \Cartalyst\Sentry\Units\ProviderInterface
             * @return void
             */
            public function setUnitProvider(UnitProviderInterface $unitProvider)
            {
                $this->unitProvider = $unitProvider;
            }

            /**
             * Gets the unit provider for Sentry.
             *
             * @return \Cartalyst\Sentry\Units\ProviderInterface
             */
    ...
    {
        return $this->unitProvider->findById($id);
    }
    ...
    {
        return $this->unitProvider->findByName($name);
    }
    ...
    {
        return $this->unitProvider->findAll();
    }
    ...
    {
        return $this->unitProvider->create($attributes);
    }
     ...
     * a unit.
     *
     * @param  \Cartalyst\Sentry\Units\UnitInterface  $unit
     * @return array
     */
    public function findAllUsersInUnit($unit)
    {
        return $this->userProvider->findAllInUnit($unit);
    }
